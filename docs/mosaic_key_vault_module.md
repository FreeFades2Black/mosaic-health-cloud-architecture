...
- **`main.tf`** — Resources: `azurerm_key_vault`, `azurerm_key_vault_key`, `azurerm_private_endpoint`, and `azurerm_monitor_diagnostic_setting`.
- **`variables.tf`** — Validated parameters with custom regex, SKU enforcement (`premium`), and retention constraints.
- **`outputs.tf`** — Key Vault ID, URI, versioned key ID, versionless key ID, and private endpoint IP.
- **`terraform.tfvars.example`** — Sanitized enterprise variables template.
